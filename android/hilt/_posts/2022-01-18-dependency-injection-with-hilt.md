...
---
Hilt 를 사용하는 모든 앱들은 `@HiltAndroidApp` 애노테이션을 Application 클래스에 포함시켜야 한다.
...
---
한번 Application 클래스에 Hilt 가 설정하고 Application 단계의 컴포넌트를 사용가능해지면 Hilt는 다른 `@AndroidEntryPoint` 애노테이션을 가진 안드로이드 클래스에 의존성을 제공한다.
...
만약 @AndroidEntryPoint 를 안드로이드 클래스에 적용했으면 이 안드로이드 클래스를 의존하는 다른 안드로이드 클래스 또한 @AndroidEntryPoint 애노테이션을 붙여야한다. 예를 들면 fragment 에 붙였으면 해당 프래그먼트를 사용하는 다른 activity 들에도 적용해야 하는것이다.
...
필드에 의존성을 주입받고 싶으면 @Inject 애노테이션을 사용하면 Hilt 컴포넌트로 부터 의존성을 얻을 수 있다.
...
Hilt 에 바인딩 정보를 제공하는 한가지 방법으로 생성자 주입이 있다. `@Inject` 애노테이션을 생성자에 쓰고 
...
해당 애노테이션이 붙은 클래스의 생성자 파라미터는 클래스의 의존성들이다. 예를 들어 AnalyicsAdapter 가 
...
인터페이스를 예시로 들자면 만약 A,B 클래스 2개의 클래스가 있고 A 클래스에 B 클래스에 대한 의존성을 가지고 A 클래스를 특정 Activity 에서 필드로 참조한다 가정해보자
...
인터페이스를 구현하는 클래스라고 가정하고 SomeActivity 에서 인터페이스에 대한 필드 주입이 이루어진다고 가정해 보자
...
이럴경우 SomeActivity 에서는 정상적으로 주입되지 않고 에러가 나게 된다. interface 가 구현화된 Type 의 객체를 어떻게 생성해야 하는지 알 수 없기 때문이다.

### Hilt Modules
---
Hilt 모듈은 `@Module` 애노테이션이 붙은 클래스로 Dagger 모듈과 유사하게 Hilt 에게 확실한 Type의 인스턴스를 제공하는 방법을 알린다. Dagger 모듈과 다른점이라면 `@InstallIn` 애노테이션을 Hilt 모듈에 붙여서 어떤 안드로이드 클래스에 각 모듈을 사용하거나 설치 할걸지 Hilt 에 알려야 한다는것이다. 
...
대신 Hilt 모듈안의 @Binds 애노테이션이 붙은 추상클래스를 만듬으로써 바인딩 정보를 Hilt 에게 제공할 수 있다. @Binds 애노테이션은 인터페이스 인스턴스 제공이 필요할때 어느 구현체를 사용할 건지 Hilt 에게 알려준다.  

@Binds 애노테이션 함수는 Hilt 에게 아래의 정보를 제공한다.
...
Hilt 모듈인 AnalyticsModule 은 `@InstallIn(ActivityComponent::class)` 애노테이션을 붙인다. ExampleActivity 에 의존성 주입을 하기를 원하기 때문이다. 이 애노테이션은 `AnalyticsModule` 안의 모든 의존성은 앱의 모든 액티비티에서 사용가능하다라는 뜻이다. 
...
Hilt 모듈안에 `@Provides` 애노테이션이 붙은 함수를 만듬으로써 Hilt 에게 인스턴스를 제공하는 방법을 알릴 수 있다.  

@Provides 애노테이션이 붙은 함수는 Hilt 에게 아래의 정보를 제공한다.
...
한정자는 멀티 바인딩 정의를 가질때 지정된 바인딩을 식별하기 위해 사용하는 애노테이션이다. 즉 한정자 애노테이션이 붙어있으면 같은 반환 Type 이라 할지라도 구별을 할 수 있다.
...
--- 
필드 주입을 수행할 수 있는 각각의 안드로이드 클래스는 `@InstallIn` 애노테이션 안의 참조할 수 있는 Hilt 컴포넌트와 관련 있다. 각 Hilt 컴포넌트는 해당하는 안드로이드 클래스의 바인딩 주입하는 역할을 가진다.  
...
아래의 표는 각 생성된 컴포넌트의 scope 애노테이션 리스트다.  
...
### 컴포넌트 기본 바인딩 
---
각 Hilt 컴포넌트에는 Hilt가 사용자 정의 바인딩에 종속성으로 주입할 수 있는 기본 바인딩 세트가 함께 제공됩니다. 주의할 점은 이런 바인딩은 일반적인 Activity 와 Fragment Type 에만 해당하지 특정 하위 클래스 등에는 적용되지 않는다. Hilt 는 모든 Activity 들에 주입하기 위해 단일 Activity 컴포넌트를 정의하여 사용한다. 각 Activity 는 이 컴포넌트의 다른 인스턴스를 가진다. 
